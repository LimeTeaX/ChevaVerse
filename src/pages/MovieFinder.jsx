...
export default function MovieFinder({ searchQuery, onSearchChange }) {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      setLoadingHome(true);
      try {
        const [trendData, animeData, topData, upData] = await Promise.all([
          tmdbApi.getTrending(1),
          tmdbApi.getTVByGenre('Animation', 1),
          tmdbApi.getMoviesByGenre(28, 1),
          tmdbApi.getMoviesByGenre(16, 1),
        ]);
        setTrending(trendData.results?.slice(0, 14) || []);
        setAnime(animeData.results?.slice(0, 14) || []);
        setTopRated(topData.results?.slice(0, 14) || []);
        setUpcoming(upData.results?.slice(0, 14) || []);
      } catch (e) {
        console.error(e);
      }
  ...
  return (
    <div className="flex-1 min-w-0">
      ...
      {isSearching ? (
        ...
        /* Home mode - icon props sudah dihapus karena icon sekarang dari dalam MovieRow */
        <div>
